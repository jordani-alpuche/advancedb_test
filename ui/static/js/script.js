$(document).ready(function () {
  ...
  $(document).ready(function () {
    ...
    // Select all tables with the class 'datanew' on the current page
    $(".datanew").each(function () {
      const $table = $(this); // Process each table individually

      // No need to check for empty rows here, DataTables handles it.

      // Initialize DataTables directly
      $table.DataTable({
        bFilter: true,
        sDom: "fBtlpi", // Consider if this DOM structure is needed for all tables
        pagingType: "numbers",
        ordering: true,
        language: {
          search: " ",
          ...
          // ✨ ADD THIS LINE ✨ - Customize the message as needed
          emptyTable: "No data available in this table",
        },
        initComplete: (settings, json) => {
          // These lines might need adjustment if #tableSearch or .search-input
          // don't exist or are duplicated on pages with multiple tables.
          // It might be safer to find these relative to the table itself.
          const $dataTableWrapper = $table.closest(".dataTables_wrapper"); // Or appropriate parent
          $dataTableWrapper.find(".dataTables_filter").appendTo("#tableSearch"); // Check if #tableSearch is unique
          $dataTableWrapper
            .find(".dataTables_filter")
            .appendTo(".search-input"); // Check if .search-input is unique relative to the table
        },
      });
    }); // End of .each() loop
